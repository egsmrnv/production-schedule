...
export const ColumnSettingsModal: React.FC<ColumnSettingsModalProps> = ({ isOpen, onClose, columns, onSave }) => {
  ...
  const handleDelete = (index: number) => {
    ...
    if (window.confirm(`Вы уверены, что хотите удалить проект "${col.name}"? Это удалит все данные по этому проекту из всех дней!`)) {
  ...
  const handleAdd = () => {
    ...
    const newCol = { name: newColName.trim(), order: localColumns.length, id: `new-${Date.now()}` };
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody} style={{ maxHeight: '400px', overflowY: 'auto' }}>
          <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
            Изменяйте порядок, удаляйте неактуальные проекты или добавляйте новые.
          </p>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '16px', borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
            ...
            />
            <button className={styles.saveBtn} onClick={handleAdd}>Добавить</button>
          </div>
        </div>
